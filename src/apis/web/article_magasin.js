...
app.post("/add/article/magasin", [authJwt.verifyToken], async (req, res, next) => {
  try {
    await app.db
      .table("article_magasin")
      .insert({
        id: uuid.v1().toLocaleUpperCase(),
        description: req.body.description,
        defaultvalue: req.body.value,
        code: "ART_" +Math.random().toString(36).substring("A",7)
      })
      .then(() => {
        res.status(201).json({
 ...
 *
 */
app.put("/add/article/magasin/:id", [authJwt.verifyToken], async (req, res, next) => {
  try {
    if (req.body) {
      await app.db
        .table("article_magasin")
...
app.get("/articles/magasing", [authJwt.verifyToken], async (req, res, next) => {
  try {
    await app.db
      .from("article_magasin")
      .select("*")
      .then((rows) => {
        res.json({
