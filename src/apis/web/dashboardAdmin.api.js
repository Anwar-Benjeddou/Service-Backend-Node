 ...
 *
 */
app.get("/admin/colis/count", [authJwt.verifyToken], async (req, res, next) => {
  try {
    const agence_user = req.agence;
  ...
  async (req, res, next) => {
    try {
      ...
      const [exchangeTotal] = await app
        ...
        .andWhere("from_agence", "=", agence_user);
 ...
 *
 */
app.get("/console_to/count", [authJwt.verifyToken], async (req, res, next) => {
  try {
    const agence_user = req.agence;
    ...
    const [exchangeTotal] = await app
      .db("console")
      .count({ count: ["status_console"] })
      .andWhere("to_agence", "=", agence_user);
